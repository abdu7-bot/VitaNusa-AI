...
function injectShellStyles() {
  ...
  style.textContent = `
    ...
    .vn-right-rail{position:fixed;top:16px;right:16px;bottom:16px;z-index:70;width:min(328px,calc(100vw - 32px));display:flex;flex-direction:column;gap:12px;padding:14px;border:1px solid var(--vn-shell-line);border-radius:28px;background:rgba(255,255,255,.9);box-shadow:0 24px 70px rgba(20,83,45,.13);backdrop-filter:blur(18px);overflow:auto;}
    ...
    .vn-rail-link{display:flex;align-items:center;gap:10px;min-height:58px;padding:9px;border:1px solid rgba(22,163,74,.12);border-radius:18px;background:rgba(255,255,255,.72);color:var(--vn-shell-ink);text-decoration:none;transition:transform .18s ease,border-color .18s ease,background .18s ease;}
    ...
    .vn-shell-toggle{position:fixed;right:14px;bottom:14px;z-index:95;display:none;align-items:center;gap:8px;min-height:44px;padding:10px 13px;border:1px solid var(--vn-shell-line);border-radius:999px;background:#fff;color:var(--vn-shell-green);font-weight:850;box-shadow:0 16px 34px rgba(20,83,45,.16);}
    .vn-shell-overlay{position:fixed;inset:0;z-index:80;background:rgba(8,28,18,.46);backdrop-filter:blur(4px);}
